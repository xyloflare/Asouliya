...
import { updateGuildInfo, getGuildInfo } from "../../modules/database.mjs";
...
export async function execute(interaction) {
  switch (interaction.options.getSubcommand()) {
    case "level_channel":
      ...
      await updateGuildInfo(interaction.guildId, {
    ...
    case "levelup_msg":
      ...
      updateGuildInfo(interaction.guildId, { levelupMsg: lvlMsg });
      await interaction.reply({
    ...
    case "enable_level_roles":
      ...
      updateGuildInfo(interaction.guildId, {
        levelRolesEnabled: enableRoles,
      });
      await interaction.reply({
        content: `Done! Level roles are now ${enableRoles ? "enabled" : "disabled"}`,
          flags: MessageFlags.Ephemeral
      });
      break;
    ...
    case "level_system":
      ...
      await interaction.reply({
        content: `Done! Level system is now ${enableLevels ? "enabled" : "disabled"}`,
      });
      break;
    ...
    case "level_role":
      ...
      const [previousData, isCreatedNow] = await getGuildInfo(
        interaction.guildId,
      );
      ...
      if (lvlRolesObj[`level_${levelNum}`]) {
        return interaction.reply({
          content: `That level (${levelNum}) is already set to <@&${lvlRolesObj[`level_${levelNum}`]}> role`,
          flags: MessageFlags.Ephemeral
        });
      }
